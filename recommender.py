...
class SHLRecommender:
    def __init__(self, csv_path: str):
        ...
        self.df['combined_text'] = self.df.apply(
            lambda row: f"{row['assessment_name']} {row['test_type']} {row['description']} "
                        f"{row['remote_testing_support']} {row['adaptive/irt_support']}", axis=1)

        # Load pre-trained model and encode corpus
...
# Recommender instance using LRU caching for performance
@lru_cache(maxsize=1)
def get_recommender(csv_path="data/enriched_assessments.csv"):
    return SHLRecommender(csv_path)

def get_top_k_recommendations(query, k=10, csv_path="data/enriched_assessments.csv"):
